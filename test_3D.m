%% Test meshfree approximations for 3D domains
%
%  Inspection of the basis function and its first derivative
%  on a 3D solid using Moving Least Squares (MLS), Radial Point Interpolation (RPI), 
...
if irregular == 1
    [nodes, nelem] = TetMesh(l, h);
    [eval, eelem] = TetMesh(l, 0.2*h);
else
    [nodes, nelem] = HexMesh(l, h);
    [eval, eelem] = HexMesh(l, 0.2*h);
end
nodes_num = length(nodes);
eval_num = length(eval);

% Inspection point coordinates and index of nearest geometry node.
ipoint = [0.5*l, 0.5*l, 0.5*l];
...
MLS_PHIDY = zeros(eval_num, nodes_num);
MLS_PHIDZ = zeros(eval_num, nodes_num);
RPI_PHI = zeros(eval_num, nodes_num);
RPI_PHIDX = zeros(eval_num, nodes_num);
RPI_PHIDY = zeros(eval_num, nodes_num);
RPI_PHIDZ = zeros(eval_num, nodes_num);
MKI_PHI = zeros(eval_num, nodes_num);
MKI_PHIDX = zeros(eval_num, nodes_num);
MKI_PHIDY = zeros(eval_num, nodes_num);
MKI_PHIDZ = zeros(eval_num, nodes_num);

% Compute basis function and derivative for each evaluation point.
textprogressbar('Calculating 3D basis function and gradient: ');
for i = 1:eval_num
    ...
    MLS_PHIDX(i,:) = mls_dphi(:,1);
    RPI_PHIDX(i,:) = rpi_dphi(:,1);
    MKI_PHIDX(i,:) = mki_dphi(:,1);
    
    % Collect basis function Y derivative values.
    MLS_PHIDY(i,:) = mls_dphi(:,2);
    RPI_PHIDY(i,:) = rpi_dphi(:,2);
    MKI_PHIDY(i,:) = mki_dphi(:,2);
    
    % Collect basis function Z derivative values.
    MLS_PHIDZ(i,:) = mls_dphi(:,3);
    RPI_PHIDZ(i,:) = rpi_dphi(:,3);
    MKI_PHIDZ(i,:) = mki_dphi(:,3);
...
% Plot basis function.
figure; hold on;
temp = int32(eval_num^(1/3));
subplot(1,3,1);
ShowSolution3D(eval, ipoint, MLS_PHI(:,inode));
title('MLS Basis function'); view([50,10]);
subplot(1,3,2);
ShowSolution3D(eval, ipoint, RPI_PHI(:,inode));
title('RPI Basis function'); view([50,10]);
subplot(1,3,3);
ShowSolution3D(eval, ipoint, MKI_PHI(:,inode));
title('MKI Basis function'); view([50,10]);

% Plot X derivative of basis function.
figure; hold on;
subplot(1,3,1);
ShowSolution3D(eval, ipoint, MLS_PHIDX(:,inode));
title('MLS Basis function X derivative'); view([50,10]);
subplot(1,3,2);
ShowSolution3D(eval, ipoint, RPI_PHIDX(:,inode));
title('RPI Basis function X derivative'); view([50,10]);
subplot(1,3,3);
ShowSolution3D(eval, ipoint, MKI_PHIDX(:,inode));
title('MKI Basis function X derivative'); view([50,10]);

% Plot Y derivative of basis function.
figure; hold on;
subplot(1,3,1);
ShowSolution3D(eval, ipoint, MLS_PHIDY(:,inode));
title('MLS Basis function Y derivative'); view([50,10]);
subplot(1,3,2);
ShowSolution3D(eval, ipoint, RPI_PHIDY(:,inode));
title('RPI Basis function Y derivative'); view([50,10]);
subplot(1,3,3);
ShowSolution3D(eval, ipoint, MKI_PHIDY(:,inode));
title('MKI Basis function Y derivative'); view([50,10]);

% Plot Z derivative of basis function.
figure; hold on;
subplot(1,3,1);
ShowSolution3D(eval, ipoint, MLS_PHIDZ(:,inode));
title('MLS Basis function Z derivative'); view([50,10]);
subplot(1,3,2);
ShowSolution3D(eval, ipoint, RPI_PHIDZ(:,inode));
title('RPI Basis function Z derivative'); view([50,10]);
subplot(1,3,3);
ShowSolution3D(eval, ipoint, MKI_PHIDZ(:,inode));
title('MKI Basis function Z derivative'); view([50,10]);
