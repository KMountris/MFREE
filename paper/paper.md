...
where $\bm{P}$ is the polynomial basis moment matrix:
...
where $\bm{P}$ is the polynomial basis moment matrix defined as in equation (\ref{eq:poly_moment}) and $\bm{R}$ is the radial basis function moment matrix:
\begin{equation}
    \bm{R} = \left[
             \begin{matrix}
